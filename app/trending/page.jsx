...
export default async function TrendingPage() {
  ...
  return (
    <main style={{
      ...
      {/* GRID — 3 kolom, poster lebih kecil */}
            ...
            <a key={f.id} href={`/watch/${f.id}`} style={{ textDecoration:'none' }}>
              <div style={{
                ...
                <div style={{ position:'relative', aspectRatio:'2/3' }}>
                  ...
                  {/* Rank badge top 3 */}
                  {idx < 3 && (
                    <div style={{ position:'absolute', top:6, left:6, width:20, height:20, borderRadius:6, background: idx===0?'linear-gradient(135deg,#ffd700,#ff8c00)': idx===1?'linear-gradient(135deg,#c0c0c0,#808080)':'linear-gradient(135deg,#cd7f32,#8b4513)', display:'flex', alignItems:'center', justifyContent:'center', fontSize:10, fontWeight:900, color:'#fff', boxShadow:'0 2px 6px rgba(0,0,0,0.3)' }}>
                      {idx+1}
                    </div>
                  )}

                  {/* Rating */}
                  <div style={{ position:'absolute', top:6, right:6, background:'rgba(0,0,0,0.65)', backdropFilter:'blur(6px)', borderRadius:6, padding:'2px 6px', fontSize:10, fontWeight:700, color:'#fbbf24', display:'flex', alignItems:'center', gap:2 }}>
                    <svg width="8" height="8" viewBox="0 0 24 24" fill="#fbbf24"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"/></svg>
                ...
                <div style={{ padding:'7px 8px 9px' }}>
                  <div style={{ fontSize:11, fontWeight:800, color:'#1a237e', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{f.title||f.name}</div>
