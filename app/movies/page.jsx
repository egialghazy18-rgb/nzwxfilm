...
const SORTS = [
  ...
  { label: '📅 Terbaru', val: 'release_date.desc' },
  { label: '📅 Terlama', val: 'release_date.asc' },
];
...
export default function MoviesPage() {
  ...
  useEffect(() => {
    fetch(`https://api.themoviedb.org/3/genre/movie/list?api_key=${KEY}&language=id-ID`)
  ...
  return (
    <main style={{ minHeight: '100vh', background: bg, paddingBottom: 100 }}>
      <div style={{ maxWidth: 900, margin: '0 auto', padding: '60px 16px 80px' }}>
        <h1 style={{ fontSize: 22, fontWeight: 800, color: txt, marginBottom: 20 }}>🎬 Film</h1>
        ...
        <div style={{ marginBottom: 14 }}>
          ...
          <div style={{ display: 'flex', gap: 8, overflowX: 'auto', paddingBottom: 4 }}>
            {SORTS.map(s => (
              <button key={s.val} onClick={() => setSort(s.val)} style={chip(sort === s.val)}>{s.label}</button>
            ))}
          </div>
        </div>
        ...
        <div style={{ marginBottom: 20 }}>
          ...
          <div style={{ display: 'flex', gap: 8, overflowX: 'auto', paddingBottom: 4 }}>
            ...
            {genres.map(g => (
              <button key={g.id} onClick={() => setGenre(String(g.id))} style={chip(genre === String(g.id))}>{g.name}</button>
            ))}
          </div>
        </div>
