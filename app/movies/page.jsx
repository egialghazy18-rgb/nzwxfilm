...
const SORTS = [
  ...
  { label: 'Terbaru', val: 'release_date.desc', icon: <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="4" width="18" height="18" rx="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg> },
  { label: 'Terlama', val: 'release_date.asc', icon: <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="4" width="18" height="18" rx="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg> },
];
...
export default function MoviesPage() {
  ...
  useEffect(() => {
    fetch(`https://api.themoviedb.org/3/genre/movie/list?api_key=${KEY}&language=id-ID`).then(r=>r.json()).then(d=>setGenres(d.genres||[]));
  }, []);

  useEffect(() => { setMovies([]); setPage(1); setHasMore(true); }, [genre, sort]);

  useEffect(() => {
    setLoading(true);
    const params = new URLSearchParams({ api_key:KEY, language:'id-ID', sort_by:sort, page, 'vote_count.gte':'50', ...(genre&&{with_genres:genre}) });
    fetch(`https://api.themoviedb.org/3/discover/movie?${params}`).then(r=>r.json()).then(d=>{
    ...
    <main style={{ minHeight:'100vh',background:bg,paddingBottom:100 }}>
      <div style={{ maxWidth:900,margin:'0 auto',padding:'60px 16px 80px' }}>
        <div style={{ display:'flex',alignItems:'center',gap:10,marginBottom:20 }}>
          <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke={dark?'#4fc3f7':'#1a237e'} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="2" y="2" width="20" height="20" rx="2"/><line x1="7" y1="2" x2="7" y2="22"/><line x1="17" y1="2" x2="17" y2="22"/><line x1="2" y1="12" x2="22" y2="12"/></svg>
          <h1 style={{ fontSize:22,fontWeight:800,color:txt }}>Film</h1>
        </div>
          ...
          {movies.map(f=>(
            <a key={f.id} href={`/watch/${f.id}`} style={{ display:'block',textDecoration:'none' }}>
              <div style={{ borderRadius:14,overflow:'hidden',position:'relative',aspectRatio:'2/3',background:dark?'#1a2744':'#1a3a5c' }}>
                {f.poster_path&&<img src={`${IMG}${f.poster_path}`} alt={f.title} style={{ width:'100%',height:'100%',objectFit:'cover',display:'block' }} loading="lazy" />}
