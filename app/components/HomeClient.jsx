  ...
  return (
    <section style={{ marginBottom:32, padding:'0 16px' }}>
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:14 }}>
        ...
        {href && <a href={href} style={{ fontSize:12, color:'#4fc3f7', fontWeight:700 }}>Lihat Semua →</a>}
      </div>
...
export default function HomeClient({ trending, movies, series }) {
  ...
  return (
    <>
      {sidebarOpen && (
        <div onClick={() => setSidebarOpen(false)} style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.55)', zIndex:9998, backdropFilter:'blur(3px)' }} />
      )}

      <div style={{ position:'fixed', top:0, left:0, height:'100dvh', width:285, background:sideBg, zIndex:9999, transform:sidebarOpen?'translateX(0)':'translateX(-100%)', transition:'transform 0.3s cubic-bezier(.4,0,.2,1)', boxShadow:sidebarOpen?'8px 0 40px rgba(0,0,0,0.25)':'none', display:'flex', flexDirection:'column', overflowY:'auto' }}>
        ...
        <div style={{ padding:'16px 12px', flex:1 }}>
          ...
          {MENU.map(m => {
            ...
            return (
              <a key={m.href} href={m.href} onClick={() => setSidebarOpen(false)} style={{ display:'flex', alignItems:'center', gap:12, padding:'11px 12px', borderRadius:14, marginBottom:4, textDecoration:'none', background:active?'#1565c0':'transparent', color:active?'#fff':sideTxt, transition:'all 0.15s' }}>
        ...
        {cont.length > 0 && (
          <div style={{ padding:'0 16px', marginBottom:24 }}>
            <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:12 }}>
              <h2 style={{ fontSize:19, fontWeight:900, color:dark?'#fff':'#1a237e', paddingLeft:12, borderLeft:'4px solid #4fc3f7', letterSpacing:'-0.5px' }}>Lanjut Nonton</h2>
              <a href="/history" style={{ fontSize:12, color:'#4fc3f7', fontWeight:700 }}>Lihat Semua →</a>
            </div>
