  ...
  return (
    <main style={{ minHeight: '100vh', background: bg, paddingBottom: 100 }}>
      <div style={{ maxWidth: 900, margin: '0 auto', padding: '60px 16px 80px' }}>
        ...
        {/* Tabs */}
        <div style={{ display:'flex', gap:8, marginBottom:24, background: dark ? 'rgba(255,255,255,0.05)' : 'rgba(255,255,255,0.5)', borderRadius:14, padding:4 }}>
          {tabs.map(t => (
            <button key={t.id} onClick={() => setTab(t.id)} style={{
              flex:1, display:'flex', alignItems:'center', justifyContent:'center', gap:6,
              padding:'10px 0', borderRadius:10, border:'none', cursor:'pointer',
              background: tab === t.id ? (dark ? '#1565c0' : '#1565c0') : 'transparent',
            ...
            {loading ? (
              ...
              <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
                {newFilms.map(f => {
                  ...
                  return (
                    <a key={f.id} href={`/watch/${f.id}`} style={{ display:'flex', gap:12, textDecoration:'none', background: dark ? 'rgba(255,255,255,0.05)' : 'rgba(255,255,255,0.6)', backdropFilter:'blur(12px)', borderRadius:16, padding:12, border: dark ? '1px solid rgba(255,255,255,0.08)' : '1px solid rgba(255,255,255,0.8)' }}>
                      <div style={{ width:60, height:90, borderRadius:10, overflow:'hidden', background:card, flexShrink:0 }}>
                        {poster && <img src={poster} alt={title} style={{ width:'100%', height:'100%', objectFit:'cover' }} />}
                      </div>
                      <div style={{ flex:1, minWidth:0 }}>
                        <div style={{ display:'flex', alignItems:'center', gap:6, marginBottom:4 }}>
                          <p style={{ fontSize:13, fontWeight:700, color:txt, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{title}</p>
                        ...
                        <div style={{ display:'flex', alignItems:'center', gap:6, marginBottom:6 }}>
                          ...
                          <span style={{ width:3, height:3, borderRadius:'50%', background:sub }} />
                          <span style={{ fontSize:11, color: f._type === 'tv' ? '#8e24aa' : '#e53935', fontWeight:600 }}>{f._type === 'tv' ? 'SERIES' : 'FILM'}</span>
                        </div>
